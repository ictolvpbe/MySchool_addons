...
class ProfessionaliseringAddress(models.Model):
    _name = 'myschool_professionalisering.address'
    ...
    def action_mark_reviewed(self):
        ...
        for rec in self:
            ...
            # Sluit eventuele openstaande myschool_activiteiten af
    ...
    def _notify_directie_for_review(self, source_record=None):
        ...
        list_url = (
            '/odoo/action-myschool_professionalisering.action_professionalisering_addresses_review'
        )
        for rec in self:
            ...
            for user in directie_users:
                ...
                self.env['bus.bus']._sendone(
                    user.partner_id,
                    'myschool_professionalisering_address_review',
                    {
    ...
    def _get_picker(self):
        Picker = self.env['myschool_professionalisering.address.picker']
...
class IrActionsActWindow(models.Model):
    ...
    def _get_action_dict(self):
        ...
        if result.get('res_model') == 'myschool_professionalisering.record':
            ...
            if not any(k.startswith('search_default_') for k in ctx):
                user = self.env.user
                if user.has_group('myschool_professionalisering.group_professionalisering_admin'):
                    pass
                elif user.has_group('myschool_professionalisering.group_professionalisering_boekhouding'):
                    ctx['search_default_state_done'] = 1
                elif user.has_group('myschool_professionalisering.group_professionalisering_directie'):
                    pass
                elif user.has_group('myschool_professionalisering.group_professionalisering_vervangingen'):
...
class ProfessionaliseringRecord(models.Model):
    _name = 'myschool_professionalisering.record'
    ...
    def _auto_init(self):
        ...
        if not Sequence.with_context(active_test=False).search_count([('code', '=', 'myschool_professionalisering.record')]):
            Sequence.create({
                'name': 'Professionalisering',
                'code': 'myschool_professionalisering.record',
                'prefix': 'PR-',
                'padding': 5,
                'number_next': 1,
                'number_increment': 1,
    ...
    invite_ids = fields.One2many('myschool_professionalisering.invite', 'professionalisering_id', string='Uitnodigingen')
    ...
    def _compute_can_edit_s_code(self):
        is_boekhouding_or_admin = (
            self.env.user.has_group('myschool_professionalisering.group_professionalisering_boekhouding')
            or self.env.user.has_group('myschool_professionalisering.group_professionalisering_admin')
        )
        for record in self:
    ...
    date_line_ids = fields.One2many('myschool_professionalisering.date.line', 'professionalisering_id', string='Datums')
    ...
    kosten_ids = fields.One2many(
        'myschool_professionalisering.kosten.line', 'professionalisering_id',
        string='Kosten',
    )
    ...
    def _compute_allowed_directie_json(self):
        group = self.env.ref('myschool_professionalisering.group_professionalisering_directie', raise_if_not_found=False)
        if group:
    ...
    def _compute_is_admin(self):
        is_admin = self.env.user.has_group('myschool_professionalisering.group_professionalisering_admin')
        for record in self:
            record.is_admin = is_admin

    @api.depends_context('uid')
    def _compute_is_directie(self):
        is_directie = self.env.user.has_group('myschool_professionalisering.group_professionalisering_directie')
        for record in self:
    ...
    def action_submit(self):
        ...
        needs_notification = self.env['myschool_professionalisering.record']
        for record in self:
        ...
        if needs_notification:
            needs_notification._send_notification('myschool_professionalisering.email_template_notify_directie')
    ...
    def _get_details_action(self):
        return {
            ...
            'res_id': self.id,
            'view_mode': 'form',
            'views': [(self.env.ref('myschool_professionalisering.view_professionalisering_form_details').id, 'form')],
            'target': 'new',
        }
    ...
    def action_open_address_picker(self):
        ...
        picker = self.env['myschool_professionalisering.address.picker'].create({
            'record_id': self.id,
        })
        return {
            'type': 'ir.actions.act_window',
            'name': 'Geavanceerd kiezen',
            'res_model': 'myschool_professionalisering.address.picker',
            'res_id': picker.id,
            'view_mode': 'form',
            'target': 'new',
        }
    ...
    def action_approve(self):
        ...
        for record in self:
            ...
            # Inform boekhouding about new approved myschool_professionalisering with costs
            if record.needs_payment:
                record._notify_boekhouding_new()
        self._send_notification('myschool_professionalisering.email_template_notify_employee_approved')
        self._schedule_vervangingen_activity()

    def _notify_boekhouding_new(self):
        """Inform boekhouding via activity that a new approved myschool_professionalisering exists."""
        self.ensure_one()
        ...
        for user in boekhouding_group.user_ids:
            self.activity_schedule(
                ...
                summary='Nieuwe myschool_professionalisering goedgekeurd: %s' % self.titel,
                note=(
                    'Een nieuwe myschool_professionalisering is goedgekeurd door directie. '
    ...
    def action_reject(self):
        ...
        for record in self:
            ...
            record.directie_id = self.env.user.employee_ids[:1]
        self._send_notification('myschool_professionalisering.email_template_notify_employee_rejected')
    ...
    def _schedule_bewijs_activity_single(self):
        ...
        self.activity_schedule(
            ...
            note='Uw myschool_professionalisering "%s" is afgerond. '
    ...
    def _cron_reminder_bewijs(self):
        ...
        weeks_after = int(self.env['ir.config_parameter'].sudo().get_param(
            'myschool_professionalisering.reminder_bewijs_weeks', 2))
        repeat_interval = int(self.env['ir.config_parameter'].sudo().get_param(
            'myschool_professionalisering.reminder_repeat_days', 7))
        today = fields.Date.today()
        ...
        for rec in records:
            ...
            rec._send_notification(
                'myschool_professionalisering.email_template_reminder_bewijs')
    ...
    def _schedule_vervangingen_activity(self):
        vervangingen_group = self.env.ref(
            'myschool_professionalisering.group_professionalisering_vervangingen', raise_if_not_found=False)
        ...
        for record in self:
            for user in vervangingen_group.user_ids:
                record.activity_schedule(
                    ...
                    note='De myschool_professionalisering "%s" van %s is goedgekeurd. '
    ...
    def action_delete(self):
        ...
        return {
            ...
            'params': {
                'fallback_action': 'myschool_professionalisering.action_professionalisering_main',
            },
        }
    ...
    def action_open_add_date_wizard(self):
        ...
        self.ensure_one()
        wiz = self.env['myschool_professionalisering.add.date.wizard'].create({
            'record_id': self.id,
        })
        return {
            'type': 'ir.actions.act_window',
            'name': 'Datum toevoegen',
            'res_model': 'myschool_professionalisering.add.date.wizard',
            'res_id': wiz.id,
            'view_mode': 'form',
            'target': 'new',
        }
    ...
    def _notify_directie_popup(self):
        ...
        directie_group = self.env.ref(
            'myschool_professionalisering.group_professionalisering_directie', raise_if_not_found=False)
        if not directie_group:
            return
        for record in self:
            directie_users = directie_group.user_ids.filtered(
            ...
            for user in directie_users:
                record.activity_schedule(
                    'mail.mail_activity_data_todo',
                    user_id=user.id,
                    summary='Nieuwe aanvraag myschool_professionalisering',
                ...
                self.env['bus.bus']._sendone(
                    ...
                    'simple_notification',
                    {
                        'title': 'Nieuwe myschool_professionalisering aanvraag',
...
class ProfessionaliseringInvite(models.Model):
    _name = 'myschool_professionalisering.invite'
...
class ProfessionaliseringKostenLine(models.Model):
    _name = 'myschool_professionalisering.kosten.line'
    _description = 'Professionalisering Kostenlijn'

    professionalisering_id = fields.Many2one(
        'myschool_professionalisering.record', string='Professionalisering',
...
class ProfessionaliseringDateLine(models.Model):
    _name = 'myschool_professionalisering.date.line'
    _description = 'Professionalisering Datum'
    _order = 'date'

    professionalisering_id = fields.Many2one('myschool_professionalisering.record', required=True, ondelete='cascade')
...
class ProfessionaliseringAddressPicker(models.TransientModel):
    _name = 'myschool_professionalisering.address.picker'
    _description = 'Geavanceerd adres kiezen'

    record_id = fields.Many2one(
        'myschool_professionalisering.record', string='Aanvraag', required=True,
    )
    ...
    available_org_address_ids = fields.Many2many(
        'myschool_professionalisering.address', 'pp_picker_org_rel', 'picker_id', 'address_id',
    ...
    available_address_ids = fields.Many2many(
        'myschool_professionalisering.address', 'pp_picker_loc_rel', 'picker_id', 'address_id',
    ...
    address_id = fields.Many2one(
        'myschool_professionalisering.address', string='Geselecteerd adres',
    )
    ...
    def _compute_available_org_address_ids(self):
        ...
        Address = self.env['myschool_professionalisering.address']
        for rec in self:
    ...
    def action_create_placeholder(self):
        ...
        address = self.env['myschool_professionalisering.address'].sudo().create({
            'name': loc,
