...
class ProfessionaliseringImportWizard(models.TransientModel):
    _name = 'myschool_professionalisering.import.wizard'
    ...
    def _import_records(self, records_data):
        ...
        Vak = self.env['myschool_professionalisering.vak'].sudo()
        Address = self.env['myschool_professionalisering.address'].sudo()
...
class ProfessionaliseringImportRollback(models.TransientModel):
    ...
    _name = 'myschool_professionalisering.import.rollback'
    ...
    def _compute_legacy_count(self):
        for rec in self:
            rec.legacy_count = self.env['myschool_professionalisering.record'].search_count(
                [('legacy_id', '!=', False)]
            )

    def action_rollback(self):
        self.ensure_one()
        records = self.env['myschool_professionalisering.record'].sudo().search(
            [('legacy_id', '!=', False)]
        )
        n = len(records)
        records.unlink()
