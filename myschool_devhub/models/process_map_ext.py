...
class ProcessMapStepDevhub(models.Model):
    _inherit = 'process.map.step'

    devhub_item_id = fields.Many2one(
...
class ProcessMapDevhub(models.Model):
    ...
    def _build_prompt(self, module_name):
        ...
        linked_stories = self.step_ids.mapped('devhub_item_id').filtered(
            lambda s: s.item_type == 'story')

        # Find the parent project (if any) and collect unlinked stories
        project = self.env['devhub.project'].search(
            [('process_map_ids', 'in', self.ids)], limit=1)
        unlinked_stories = self.env['devhub.item']
        if project:
        ...
        if linked_stories:
            ...
            for story in linked_stories.sorted('sequence'):
                ...
                steps = self.step_ids.filtered(
                    lambda s: s.devhub_item_id.id == story.id)
        ...
        for step in self.step_ids:
            if step.devhub_item_id:
                step_story_map[step.name] = step.devhub_item_id.display_name
    ...
    def get_diagram_data(self):
        ...
        for step_data in data['steps']:
            ...
            if step:
                step_data['devhub_item_id'] = step.devhub_item_id.id if step.devhub_item_id else False
                step_data['devhub_item_name'] = step.devhub_item_id.display_name if step.devhub_item_id else ''
        return data

    def save_diagram_data(self, data):
        # Collect devhub_item_id mapping before super processes steps
        devhub_by_id = {}
        devhub_by_attrs = {}
        for step_data in data.get('steps', []):
            if 'devhub_item_id' not in step_data:
                continue
            sid = step_data.get('id')
            item_id = step_data.get('devhub_item_id') or False
            if isinstance(sid, int) and sid > 0:
                devhub_by_id[sid] = item_id
            else:
                ...
                devhub_by_attrs[key] = item_id

        result = super().save_diagram_data(data)

        # Apply devhub_item_id to existing steps (matched by ID)
        Step = self.env['process.map.step']
        for sid, item_id in devhub_by_id.items():
            step = Step.browse(sid).exists()
            if step:
                step.devhub_item_id = item_id

        # Apply devhub_item_id to newly created steps (matched by attributes)
        if devhub_by_attrs:
            for step in self.step_ids:
                if step.devhub_item_id:
                    continue
                key = (step.name, round(step.x_position), round(step.y_position))
                item_id = devhub_by_attrs.get(key)
                if item_id:
                    step.devhub_item_id = item_id
    ...
    def search_user_stories(self, query='', project_id=False):
        ...
        stories = self.env['devhub.item'].search(domain, limit=20, order='sequence')
    ...
    def create_user_story(self, name, project_id):
        ...
        story = self.env['devhub.item'].create({
            'name': name,
            'item_type': 'story',
