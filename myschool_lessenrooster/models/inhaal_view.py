"""Backend voor de 'Mijn myschool_lessenrooster — inhaalplanning'-view.
...
Een Owl client-action haalt z'n data via deze methods op:
  ...
  - create_inhaal(klas_id, date, lesuur, gemiste_les_id)
                              → maakt myschool_planner.record + notificatie naar beheerder
"""
...
class LessenroosterInhaalView(models.AbstractModel):
    ...
    _name = 'myschool_lessenrooster.inhaal.view'
    ...
    def _absence_dates_for(self, employee, klas):
        ...
        klas: lijst van bron-myschool_activiteiten."""
        result = {}
        person = employee and self.env['myschool.person'].search(
            [('odoo_employee_id', '=', employee.id)], limit=1)

        # Leerkracht via myschool_activiteiten
        if person:
            acts = self.env['myschool_activiteiten.record'].sudo().search([
        ...
        # Leerkracht via myschool_professionalisering
        if employee and 'myschool_professionalisering.record' in self.env:
            profs = self.env['myschool_professionalisering.record'].sudo().search([
            ...
            for pr in profs:
                ...
                while cur <= end_date:
                    bucket = result.setdefault(
                        cur, {'leerkracht': [], 'klas': []})
                    bucket['leerkracht'].append({
                        'type': 'myschool_professionalisering',
                        'titel': pr.titel or pr.name,
                    })
                    cur += timedelta(days=1)

        # Klas via myschool_activiteiten
        if klas:
            acts = self.env['myschool_activiteiten.record'].sudo().search([
                ('state', 'in', ('approved', 's_code', 'vervanging', 'done')),
                ('klas_ids', 'in', [klas.id]),
    ...
    def get_grid_data(self, week_start_str):
        ...
        Line = self.env['myschool_lessenrooster.line']
    ...
    def get_free_slots(self, klas_id, date_from_str, date_to_str):
        ...
        AND klas are both free, EN waar geen ander inhaal-record (myschool_planner.record)
        ...
        if 'myschool_planner.record' in self.env:
            ...
            bookings = self.env['myschool_planner.record'].sudo().search(domain)
            for b in bookings:
    ...
    def create_inhaal(self, klas_id, date_str, lesuur, gemiste_les_id=None):
        """Maakt een myschool_planner.record aan voor een inhaalmoment.
        ...
        géén directe access op myschool_planner.tijdslot of myschool_planner.record nodig heeft —
        het systeem maakt het record namens hen aan."""
        if 'myschool_planner.record' not in self.env:
            raise UserError(
                "Module 'myschool_planner' is niet geïnstalleerd — kan geen "
        ...
        tijdslot = self.env['myschool_planner.tijdslot'].sudo().search(
        ...
        if tijdslot:
            ...
            conflict = self.env['myschool_planner.record'].sudo().search(
        ...
        titel_extra = ''
        if gemiste_les_id:
            les = self.env['myschool_lessenrooster.line'].sudo().browse(gemiste_les_id)
            if les.exists():
        ...
        record = self.env['myschool_planner.record'].sudo().create(vals)

        # Notificatie naar leden van group_planner_admin
        admin_group = self.env.ref(
            'myschool_planner.group_planner_admin', raise_if_not_found=False)
