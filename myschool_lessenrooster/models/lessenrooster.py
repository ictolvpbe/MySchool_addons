...
class LessenroosterKlasView(models.TransientModel):
    _name = 'myschool_lessenrooster.klas.view'
    ...
    def _compute_available_klas_ids(self):
        for rec in self:
            if rec.schooljaar:
                klas_ids = self.env['myschool_lessenrooster.line'].search([
    ...
    def _compute_available_leerkracht_ids(self):
        for rec in self:
            if rec.schooljaar:
                leerkracht_ids = self.env['myschool_lessenrooster.line'].search([
                    ('schooljaar', '=', rec.schooljaar),
                    ('leerkracht_id', '!=', False),
    ...
    def _compute_available_lokaal_ids(self):
        for rec in self:
            if rec.schooljaar:
                lokaal_ids = self.env['myschool_lessenrooster.line'].search([
                    ('schooljaar', '=', rec.schooljaar),
                    ('lokaal_id', '!=', False),
    ...
    def _compute_rooster_html(self):
        ...
        for rec in self:
            ...
            lines = self.env['myschool_lessenrooster.line'].search(domain)
...
class LessenroosterLine(models.Model):
    _name = 'myschool_lessenrooster.line'
