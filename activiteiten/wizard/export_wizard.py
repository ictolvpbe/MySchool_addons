"""Xlsx-export wizard voor activiteiten.
...
class ActiviteitenExportWizard(models.TransientModel):
    _name = 'activiteiten.export.wizard'
    ...
    def action_export(self):
        ...
        Act = self.env['activiteiten.record'].sudo()
        domain = [
            ('datetime', '>=', start),
            ('datetime', '<=', end),
        ]
        ...
        if not records:
            raise UserError(
                "Geen activiteiten gevonden voor schooljaar %d-%d." % (
        ...
        # --- Sheet 1: alle activiteiten in detail ---
        ws = wb.active
