...
class MyschoolAccessPolicy(models.Model):
    _name = 'myschool.access.policy'
    ...
    def _recompute_asset_access(self):
        ...
        assets = self.env['myschool.asset'].sudo().search([])
        if assets:
            self.env.add_to_compute(
                self.env['myschool.asset']._fields['allowed_group_read_ids'],
                assets,
            )
