...
class MyschoolProcess(models.Model):
    _name = 'myschool.process'
    _description = 'Myschool Process'
    _order = 'name'
    ...
    lane_ids = fields.One2many('myschool.process.lane', 'map_id', string='Lanes')
    step_ids = fields.One2many('myschool.process.step', 'map_id', string='Steps')
    connection_ids = fields.One2many('myschool.process.connection', 'map_id', string='Connections')
    version_ids = fields.One2many('myschool.process.version', 'map_id', string='Version History')
    ...
    def get_diagram_data(self):
        ...
        # Lane presets (global)
        lane_presets = []
        for preset in self.env['myschool.process.lane.preset'].search([]):
            lane_presets.append({
                'id': preset.id,
                'name': preset.name,
    ...
    def save_diagram_data(self, data):
        ...
        Lane = self.env['myschool.process.lane']
        Step = self.env['myschool.process.step']
        Connection = self.env['myschool.process.connection']
        ...
        Field = self.env['myschool.process.field']
    ...
    def _save_field_records(self, step_rec, field_records_data, id_map):
        """Create/update/delete myschool.process.field records for a step.
    ...
    def get_versions(self):
        ...
        self.ensure_one()
        versions = self.env['myschool.process.version'].search([('map_id', '=', self.id)], order='version_number desc')
        return [{
            'id': v.id,
    ...
    def restore_version(self, version_id):
        ...
        self.ensure_one()
        version = self.env['myschool.process.version'].browse(version_id)
