...
def migrate(cr, version):
    ...
    van bus_ids.prijs. Voor bestaande myschool_activiteiten met een ingevulde bus_price
    ...
    """
    _logger.info('[myschool_activiteiten 2.0] migrating bus_price -> myschool_activiteiten.bus.prijs')
    ...
    """)
    if not cr.fetchone():
        _logger.warning('[myschool_activiteiten 2.0] activiteiten_bus.prijs column does not exist yet, skipping')
        return

    # Vind alle myschool_activiteiten met een ingevulde bus_price
    cr.execute("""
    ...
    rows = cr.fetchall()
    if not rows:
        _logger.info('[myschool_activiteiten 2.0] no records to migrate')
        return
    ...
    _logger.info(
        '[myschool_activiteiten 2.0] migrated bus_price for %d myschool_activiteiten records',
        len(rows),
    )
