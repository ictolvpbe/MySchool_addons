...
class DrukwerkConfig(models.Model):
    _name = 'myschool_drukwerk.config'
    ...
    def write(self, vals):
        ...
        if 'prijs_per_pagina' in vals:
            param.set_param('myschool_drukwerk.prijs_per_pagina', str(vals['prijs_per_pagina']))
        if 'prijs_kleur' in vals:
            param.set_param('myschool_drukwerk.prijs_kleur', str(vals['prijs_kleur']))
        if 'prijs_a3' in vals:
            param.set_param('myschool_drukwerk.prijs_a3', str(vals['prijs_a3']))
        if 'prijs_dik_papier' in vals:
            param.set_param('myschool_drukwerk.prijs_dik_papier', str(vals['prijs_dik_papier']))
        if 'printer_naam' in vals:
            param.set_param('myschool_drukwerk.printer_naam', vals['printer_naam'] or 'Drukkerij')
        if 'count_e_artikel' in vals:
            param.set_param('myschool_drukwerk.count_e_artikel', vals['count_e_artikel'] or '')
        if 'count_e_artikel_kleur' in vals:
            param.set_param('myschool_drukwerk.count_e_artikel_kleur', vals['count_e_artikel_kleur'] or '')
        if 'count_e_analytisch' in vals:
            param.set_param('myschool_drukwerk.count_e_analytisch', vals['count_e_analytisch'] or '')
        return res
