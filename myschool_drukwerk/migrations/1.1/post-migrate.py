"""Rename existing myschool_drukwerk.record entries that are still stuck on 'New'.
...
def migrate(cr, version):
    ...
    seq = env['ir.sequence'].sudo().search(
        [('code', '=', 'myschool_drukwerk.record')], limit=1,
    )
    if not seq:
        return
    ...
    records = env['myschool_drukwerk.record'].browse(ids)
