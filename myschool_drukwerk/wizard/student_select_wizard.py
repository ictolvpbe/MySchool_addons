...
class StudentSelectLine(models.TransientModel):
    _name = 'myschool_drukwerk.student.select.line'
    _description = 'Student selectie lijn'

    wizard_id = fields.Many2one('myschool_drukwerk.student.select.wizard', required=True, ondelete='cascade')
...
class StudentSelectWizard(models.TransientModel):
    _name = 'myschool_drukwerk.student.select.wizard'
    _description = 'Leerlingen selecteren'

    drukwerk_id = fields.Many2one('myschool_drukwerk.record', required=True)
    line_ids = fields.One2many('myschool_drukwerk.student.select.line', 'wizard_id', string='Leerlingen')
    display_line_ids = fields.Many2many(
        'myschool_drukwerk.student.select.line',
    ...
    def action_confirm(self):
        """Apply selection back to the myschool_drukwerk record."""
