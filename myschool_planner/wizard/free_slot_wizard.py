...
# Mapping: Python weekday() (0-6, Monday=0) → myschool_lessenrooster.line.dag selectie ('1'-'5')
...
class PlannerFreeSlotWizard(models.TransientModel):
    ...
    (geen myschool_lessenrooster.line voor beiden op dat dag/uur)."""
    _name = 'myschool_planner.free.slot.wizard'
    _description = 'Vrij moment zoeker'

    planner_id = fields.Many2one('myschool_planner.record', required=True)
    ...
    suggestion_ids = fields.One2many(
        'myschool_planner.free.slot.suggestion', 'wizard_id',
    ...
    def action_search(self):
        ...
        # Map hr.employee → myschool.person voor myschool_lessenrooster
        person = (
            self.leerkracht_id
        ...
        while cur <= self.search_until and i < max_iterations:
            ...
            # Welke lesuren bestaan er op deze dag in het myschool_lessenrooster?
        ...
        if suggestions:
            self.env['myschool_planner.free.slot.suggestion'].create(suggestions)
...
class PlannerFreeSlotSuggestion(models.TransientModel):
    ...
    _name = 'myschool_planner.free.slot.suggestion'
    _description = 'Vrij moment suggestie'
    _order = 'date, lesuur'

    wizard_id = fields.Many2one(
        'myschool_planner.free.slot.wizard', required=True, ondelete='cascade')
    ...
    def action_apply_to_planner(self):
        """Pas deze suggestie toe op het myschool_planner-record:
        ...
        tijdslot = self.env['myschool_planner.tijdslot'].search(
