...
class PlannerRecord(models.Model):
    ...
    afwezige_id = fields.Many2one(
        'myschool_planner.afwezige.leerkracht',
        string='Afwezige',
        store=False,
    )
    ...
    vervanging_line_ids = fields.One2many(
        'myschool_planner.vervanging.line', 'planner_id',
        string='Vervangingen',
    )
    ...
    gemiste_lessen_ids = fields.Many2many(
        'myschool_lessenrooster.line',
        compute='_compute_gemiste_lessen_ids',
        string='Gemiste lessen',
    )

    # Mapping: Python weekday() (0-6, Monday=0) → myschool_lessenrooster.line.dag selectie
    ...
    def _collect_absence_dates(self):
        ...
        myschool_activiteiten en myschool_professionalisering. Returns een set van date-objects."""
        self.ensure_one()
        dates = set()
        if self.leerkracht_id:
            ...
            if person:
                acts = self.env['myschool_activiteiten.record'].search([
            ...
            if 'myschool_professionalisering.record' in self.env:
    ...
    def action_open_free_slot_wizard(self):
        ...
        return {
            ...
            'res_model': 'myschool_planner.free.slot.wizard',
            'view_mode': 'form',
            'target': 'new',
    ...
    def _compute_afwezigheid_info(self):
        ...
        for record in self:
            ...
            if record.leerkracht_id:
                ...
                if person:
                    activiteiten = self.env['myschool_activiteiten.record'].search([
                        ('state', 'in', ('approved', 's_code', 'vervanging', 'done')),
                        ('leerkracht_ids', 'in', [person.id]),
                    ], order='datetime asc')
                ...
                if 'myschool_professionalisering.record' in self.env:
                    prof_records = self.env['myschool_professionalisering.record'].sudo().search([
                        ('state', 'in', ('bevestiging', 'bewijs', 'done')),
                        '|',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == 'New':
                ...
                new_name = self.env['ir.sequence'].next_by_code('myschool_planner.record')
                if not new_name:
                    # Fallback: zoek de sequence company-onafhankelijk via sudo
                    seq = self.env['ir.sequence'].sudo().search(
                        [('code', '=', 'myschool_planner.record')], limit=1)
                    if seq:
